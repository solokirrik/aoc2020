...
def contains_sum(t_slice, t_sum):
    #     print(t_slice)
    for i, _ in enumerate(t_slice):
        for j, _ in enumerate(t_slice):
            if i == j or t_slice[i] >= t_sum or t_slice[j] >= t_sum:
                continue
            if t_sum == t_slice[i] + t_slice[j]:
                return True
    return False
...
if __name__ == '__main__':
    ...
    run = True
    while run:
        if not contains_sum(inp_e[i_l:i_r], inp_e[i_r]):
            print(inp_e[i_r])
            break
        i_l, i_r = i_l + 1, i_r + 1
    ...
    run = True
    while run:
        if not contains_sum(inp[i_l:i_r], inp[i_r]):
            print(inp[i_r])
            break
        i_l, i_r = i_l + 1, i_r + 1
