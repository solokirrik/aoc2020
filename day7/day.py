inp = """faded plum bags contain 5 wavy cyan bags.
...
inp_splitted = inp.splitlines()

def parents(set_colors, text, color):
    res_parents = {}
    for row in text:
        if not row.startswith(color) and row.count(color) > 0:
            bag_color = row[:row.find("bags")].strip()
            if bag_color not in res_parents.keys():
                res_parents[bag_color] = parents(set_colors, inp_splitted, bag_color)
                set_colors.add(bag_color)

    return res_parents

def childrens(text, p_count, p_color):
    childrens_tot = 0
    for row in text:
        if row.startswith(p_color + " bags contain "):
            raw_colors =  [x.strip() for x in row[len(p_color + " bags contain"):-1].split(", ")if x.strip() != "no other bags"]
            if len(raw_colors) == 0:
                return p_count
            colors_wo_bags = [x.rstrip(" bags").split(" ") for x in raw_colors]
            for cc in colors_wo_bags:
                count = int(cc[0])
                color = " ".join(cc[1:])
                print(count, color)
                childrens_tot +=  childrens(inp_splitted, p_count*count, color)

    return childrens_tot
...
def sum_contained_bags(color, content_map):
    contained = [
        sum_contained_bags(c, content_map) * content_map[color][c]
        for c in content_map[color]
    ]
...
if __name__=='__main__':
    ...
    data = inp.splitlines()
    for row in data:
        if not row.startswith("shiny gold") and row.count("shiny gold") > 0:
            result += 1
    ...
    res = {}
    set_colors = set()
    res[color] = parents(set_colors, inp_splitted, color)

    color = "shiny gold"
    childrens(inp_splitted, 1, color)

    color = "shiny gold"
    row = "shiny gold bags contain 4 pale black bags, 4 dim violet bags, 3 muted yellow bags."
    raw_colors =  [x.strip() for x in row[len(color + " bags contain"):-1].split(", ")]
    colors_wo_bags = [x.rstrip(" bags").split(" ") for x in raw_colors]
    row_item_color = " ".join(colors_wo_bags[0][1:])
    print(raw_colors)
    print(colors_wo_bags)
    print(row_item_color)

    row = "shiny gold bags contain 4 pale black bags, 4 dim violet bags, 3 muted yellow bags."
    color = "shiny gold"
    print(row[len(color + " bags contain"):-1])

    color = "dim olive"
    row = "dim olive bags contain no other bags."
    raw_colors =  [x.strip() for x in row[len(color + " bags contain"):-1].split(", ")if x.strip() != "no other bags" ]
    colors_wo_bags = [x.rstrip(" bags").split(" ") for x in raw_colors]
    print(raw_colors)
    print(colors_wo_bags)

    row = "dim olive bags contain no other bags."
    print(row[len(color + " bags contain"):-1].split(", "))

    print(raw_colors)
    print(colors_wo_bags)
    print(color)

    sent = "4 pale black bags"
    sent.rstrip(" bags")
    sent.rstrip(" bags")

    content_map, _ = parse_input(inp_splitted)
