...
def is_valid_by_count(inp):
    (times, letter), password = inp.split(": ")[0].split(" "), inp.split(": ")[1]
    minChar, maxChar = int(times.split("-")[0]), int(times.split("-")[1])

    instr = password.count(letter)
    return minChar <= instr <= maxChar

def is_valid_by_position(inp):
    (times,
     letter), password = inp.split(": ")[0].split(" "), inp.split(": ")[1]
    pos1, pos2 = int(times.split("-")[0]), int(times.split("-")[1])
...
if __name__ == '__main__':
    condition, password = "4-8 d: dndfcnhd".split(": ")
    times, letter = condition.split(" ")
    minChar, maxChar = int(times.split("-")[0]),int(times.split("-")[1])

    valid = 0
    for item in inp:
        if is_valid_by_count(item):
            valid += 1
